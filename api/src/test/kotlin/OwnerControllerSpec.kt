...
class OwnerControllerSpec(
    ...
    val createRequest = CreateOwnerRequest(
        userId = 1L,
        ownerName = "test",
        ownerEmail = "test@test.test",
        ownerPhoneNumber = "010-1111-1111"
    )
    val updateRequest = UpdateOwnerRequest(ownerId = 1L, name = "updateName", phoneNumber = "010-2222-3333")
            ...
            When("업주 생성을 요청하였다.") {
                And("중복된 이메일이 없으면 업주가 생성된다.") {
                    webTestClient.post().uri("/owner")
                        .body(Mono.just(createRequest), CreateOwnerRequest::class.java)
                        .exchange()
                        .expectStatus()
                        .is2xxSuccessful
                        .expectBody()
                    ...
                    Then("그리고 생성된 업주 정보를 수정하였다.") {
                        webTestClient.put().uri("/owner")
                            .body(Mono.just(updateRequest), UpdateOwnerRequest::class.java)
                            .exchange()
                            .expectStatus()
                            .is2xxSuccessful
                            .expectBody()
